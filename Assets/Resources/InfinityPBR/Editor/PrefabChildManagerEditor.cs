...
public class PrefabChildManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        prefabManager.showSetup = EditorGUILayout.Foldout(prefabManager.showSetup, "Setup & Options");
        if (prefabManager.showSetup)
        {
            EditorGUI.indentLevel++;
            prefabManager.showHelpBoxes = EditorGUILayout.Toggle("Show Help Boxes", prefabManager.showHelpBoxes);
            prefabManager.showFullInspector = EditorGUILayout.Toggle("Show Full Inspector", prefabManager.showFullInspector);
            prefabManager.instantiatePrefabsAsAdded = EditorGUILayout.Toggle("Instantiate Prefabs when Added to Group", prefabManager.instantiatePrefabsAsAdded);
                        ...
                        for (int i = 0; i < prefabManager.prefabGroups[g].prefabObjects.Count; i++)
                        {
                            ...
                            else
                            {
                                ...
                                if (oldObject != prefabObject.prefab)
                                {
                                    ...
                                    if (prefabManager.instantiatePrefabsAsAdded)
                                    {
                                        prefabManager.DeactivateGroup(g);
                                        prefabManager.ActivateGroup(g);
                                    }
        ...
        /* ------------------------------------------------------------------------------------------
         DEFAULT INSPECTOR
         ------------------------------------------------------------------------------------------*/
        if (prefabManager.showFullInspector)
        {
            EditorGUILayout.Space();
